    ...
    <section v-if="!loading" class="animated fadeIn">
      <header>
        ...
        <div class="selectList">
          ...
          <div v-if="thTitle!='累计注册数'" class="item" @click="showSelect('form')">
            <p>{{rfKey}}&nbsp;</p>
            <div :class="isShowSelect && searchType =='form'? 'jtD' : 'jt' "></div>
          </div>
        </div>
      </header>
      ...
      <div class="select animated fadeInDown" v-if="isShowSelect">
        ...
        <div v-if="searchType==='form'">
          <p @click="apiCustomerReferral({})">区域全部医院</p>
          <p @click="apiCustomerReferral(item)" v-for="(item,index) in referralItems" :key="index">
            {{
            item.name
            }}
          </p>
          ...
          <table>
            ...
            <tbody>
              <tr v-for="(item,index) in dailyDatasItems" :key="index">
                <td>{{item.date}}</td>
                <td v-if="thTitle!='累计注册数'">{{item.count}}</td>
                <td v-if="thTitle=='累计注册数'">{{item.cumulativeCount}}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  methods: {
    apiCustomer(day, ageDay) {
      ...
      if (this.rfKey != "区域全部医院") {
        req = Object.assign(req, {
          referral: {
            type: this.rfItemActive.type,
            identity: this.rfItemActive.identity
          }
        });
      }
    ...
    apiCustomerReferral(rfItem) {
      this.rfItemActive = rfItem;
      this.rfKey = rfItem.name;
      let req;

      if (rfItem.type) {
        req = {
          startDateTime: this.$utils.formatGMTDate(
            this.startDateTime + " 00:00"
          ),
          endDateTime: this.$utils.formatGMTDate(this.endDateTime + " 23:59"),
          referral: {
            type: rfItem.type,
            identity: rfItem.identity
          }
        };
      } else {
        this.rfKey = "区域全部医院";
    ...
    customerRegisterAnalysisReport(reqdata) {
      this.$http
        .post("/host/customerRegisterAnalysisReport", reqdata)
        .then(res => {
